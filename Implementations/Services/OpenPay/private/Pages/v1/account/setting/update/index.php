...
// Creates a setting (for a particular merchant account).
postedTo();
...
$row=$dz->get_row('select `ID` from `Merchant.Account.Settings` where `Account`='.$verifiedAccount.' and `Setting`="'.$setting.'"');
...
if($row){
	...
	if($value==''){
		...
		// Delete it:
		$dz->query('delete from `Merchant.Account.Settings` where ID='.$id);
		
	}else{
	
		// Update the row:
		$dz->query('update `Merchant.Account.Settings` set `Value`="'.$value.'" where `ID`='.$id);
