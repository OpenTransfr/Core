...
namespace OpenTransfr{
	...
	public static class Verifier{
		...
		/// Note that public key and signature are hex, and the message is the complete message. This uses the
		/// bitcoin double sha256 hash on the message.</summary>
		public static bool Verify(string msg,string publicKey,string signature){
			
			// Get the bytes from the hex strings:
			byte[] signatureBytes=HexString.ToBytes(signature);
			byte[] publicKeyBytes=HexString.ToBytes(publicKey);
